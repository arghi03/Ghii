...
public class BookDetailScreen extends JFrame {
    ...
    private void initComponents() {
        ...
        if (currentBook != null) {
            JLabel titleLabel = new JLabel("<html><div style='text-align: center; width: 350px;'>" + currentBook.getTitle() + "</div></html>");
            ...
            mainPanel.add(Box.createVerticalStrut(10));
            // ✅ SATU-SATUNYA BARIS YANG DITAMBAHKAN
            mainPanel.add(createDetailEntry("KLASIFIKASI DDC", currentBook.getClassificationCode(), detailHeaderFont, detailValueFont));
            mainPanel.add(Box.createVerticalStrut(10));
    ...
    private void styleActionButton(JButton button, Color bgColor, int width, int height) {
        ...
        button.addMouseListener(new MouseAdapter() {
            public void mouseEntered(MouseEvent evt) {
                if(button.isEnabled()) button.setBackground(originalBgColor.darker());
            }

            public void mouseExited(MouseEvent evt) {
                if(button.isEnabled()) button.setBackground(originalBgColor);
            }
        });
    }
    ...
    private void updateFavoriteButtonState() {
        ...
        if (isFav) {
            favoriteButton.setText("Hapus Favorit");
